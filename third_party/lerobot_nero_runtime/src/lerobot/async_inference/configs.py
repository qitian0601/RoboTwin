...
@dataclass
class AsyncRTCDebugDumpConfig:
    """Optional lightweight RTC debug dump written by the async policy server."""

    enabled: bool = field(default=False, metadata={"help": "Write lightweight RTC debug stats as JSONL"})


@dataclass
class AsyncRTCConfig:
    """Runtime RTC switch for the async policy server.

    This is separate from the policy checkpoint's rtc_config so existing async
    server commands keep their historical behavior unless RTC is explicitly
    enabled for this server process.
    """

    enabled: bool = field(
        default=False,
        metadata={"help": "Enable Real-Time Chunking guidance in the async policy server"},
    )
    latency_quantile: float = field(
        default=0.95,
        metadata={
            "help": "Rolling latency quantile used to convert async RTC inference latency to delay steps"
        },
    )
    debug_dump: AsyncRTCDebugDumpConfig = field(default_factory=AsyncRTCDebugDumpConfig)
...
class PolicyServerConfig:
    ...
    obs_similarity_atol: float = field(
        default=1.0,
        metadata={
            "help": "State-norm tolerance below which observations are considered too similar for inference"
        },
    )
    async_rtc: AsyncRTCConfig = field(default_factory=AsyncRTCConfig)
    ...
    def __post_init__(self):
        ...
        if not 0.0 <= self.async_rtc.latency_quantile <= 1.0:
            raise ValueError(
                f"async_rtc.latency_quantile must be between 0 and 1, got {self.async_rtc.latency_quantile}"
            )
    ...
    def from_dict(cls, config_dict: dict) -> "PolicyServerConfig":
        ...
        if isinstance(config_dict.get("async_rtc"), dict):
            async_rtc = dict(config_dict["async_rtc"])
            if isinstance(async_rtc.get("debug_dump"), dict):
                async_rtc["debug_dump"] = AsyncRTCDebugDumpConfig(**async_rtc["debug_dump"])
            config_dict["async_rtc"] = async_rtc
            config_dict["async_rtc"] = AsyncRTCConfig(**config_dict["async_rtc"])
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "async_rtc": {
                "enabled": self.async_rtc.enabled,
                "latency_quantile": self.async_rtc.latency_quantile,
                "debug_dump": {"enabled": self.async_rtc.debug_dump.enabled},
            },
        }
...
class RobotClientConfig:
    ...
    record_dir: str = field(default="", metadata={"help": "Run directory where server-side debug data is saved"})
