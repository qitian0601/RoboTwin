...
def _skew_numpy(vectors: np.ndarray) -> np.ndarray:
...
def _rotvec_to_matrix_numpy(rotvec: np.ndarray) -> np.ndarray:
    original_shape = rotvec.shape[:-1]
    flat = rotvec.reshape(-1, 3).astype(np.float64)
    ...
    one_minus_cos_over_theta2 = np.where(
        small,
        0.5 - theta2 / 24 + theta2 * theta2 / 720,
        (1 - np.cos(theta)) / np.maximum(theta2, 1e-12),
    )
    eye = np.broadcast_to(np.eye(3, dtype=np.float64), (flat.shape[0], 3, 3))
    matrix = eye + sin_over_theta[..., None] * k + one_minus_cos_over_theta2[..., None] * k2
    return matrix.reshape(*original_shape, 3, 3)


def _matrix_to_rotvec_numpy(matrix: np.ndarray) -> np.ndarray:
    original_shape = matrix.shape[:-2]
    flat = matrix.reshape(-1, 3, 3).astype(np.float64)
    ...
    scale = np.empty((flat.shape[0], 1), dtype=np.float64)
    small = angle < 1e-6
    near_pi = np.pi - angle < 1e-5
    regular = ~(small | near_pi)
    scale[small] = 0.5 + (angle[small, None] ** 2) / 12
    scale[regular] = angle[regular, None] / (2 * np.sin(angle[regular, None]))

    rotvec = vee * scale
    if np.any(near_pi):
        for idx in np.flatnonzero(near_pi):
            rotvec[idx] = _near_pi_matrix_to_rotvec(flat[idx], angle[idx])
    return rotvec.reshape(*original_shape, 3)


def _near_pi_matrix_to_rotvec(matrix: np.ndarray, angle: float) -> np.ndarray:
    axis = np.empty(3, dtype=np.float64)
    diag = np.diag(matrix)
    idx = int(np.argmax(diag))
    axis[idx] = np.sqrt(max(diag[idx] + 1.0, 0.0) / 2.0)
    denom = max(2.0 * axis[idx], 1e-12)
    if idx == 0:
        axis[1] = (matrix[0, 1] + matrix[1, 0]) / denom
        axis[2] = (matrix[0, 2] + matrix[2, 0]) / denom
    elif idx == 1:
        axis[0] = (matrix[0, 1] + matrix[1, 0]) / denom
        axis[2] = (matrix[1, 2] + matrix[2, 1]) / denom
    else:
        axis[0] = (matrix[0, 2] + matrix[2, 0]) / denom
        axis[1] = (matrix[1, 2] + matrix[2, 1]) / denom
...
def _relative_ee_so3_actions(actions: np.ndarray, states: np.ndarray) -> np.ndarray:
    action_right, action_left = actions[..., :7], actions[..., 7:14]
    state_right, state_left = states[..., :7], states[..., 7:14]

    def convert_arm(action_arm: np.ndarray, state_arm: np.ndarray) -> np.ndarray:
        pos_delta = action_arm[..., :3] - state_arm[..., :3]
        r_target = _rotvec_to_matrix_numpy(action_arm[..., 3:6])
        r_current = _rotvec_to_matrix_numpy(state_arm[..., 3:6])
        r_delta = r_target @ np.swapaxes(r_current, -1, -2)
        rot_delta = _matrix_to_rotvec_numpy(r_delta)
        gripper = action_arm[..., 6:7]
        return np.concatenate((pos_delta, rot_delta, gripper), axis=-1)

    return np.concatenate((convert_arm(action_right, state_right), convert_arm(action_left, state_left)), axis=-1)


def _compute_ee_so3_relative_chunk_batch(
    start_indices: np.ndarray,
    all_actions: np.ndarray,
    all_states: np.ndarray,
    chunk_size: int,
    ...
    offsets = np.arange(chunk_size)
    frame_idx = start_indices[:, None] + offsets[None, :]
    chunks = all_actions[frame_idx]
    states = np.broadcast_to(all_states[start_indices, None, :], chunks.shape)
    return _relative_ee_so3_actions(chunks, states).reshape(-1, 14).astype(np.float32)


def compute_ee_so3_relative_action_stats(
    hf_dataset,
    features: dict,
    chunk_size: int,
    num_workers: int = 0,
) -> dict[str, np.ndarray]:
    """Compute relative EE/SO(3) action stats for 14-D dual-arm EE pose datasets."""
    action_dim = features[ACTION]["shape"][0]
    state_dim = features[OBS_STATE]["shape"][0]
    if action_dim != 14 or state_dim != 14:
        raise ValueError(f"EE/SO3 relative stats require 14-D action/state, got {action_dim}/{state_dim}")

    logging.info("Loading EE action/state data for SO(3) relative action stats...")
    ...
    logging.info(
        f"Computing EE/SO3 relative action stats from {len(valid_starts)} chunks "
    ...
    if num_workers > 1:
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as pool:
            futures = [
                pool.submit(
                    _compute_ee_so3_relative_chunk_batch,
                    batch,
                    all_actions,
                    all_states,
                    chunk_size,
        ...
        for batch in batches:
            running_stats.update(
                _compute_ee_so3_relative_chunk_batch(batch, all_actions, all_states, chunk_size)
            )

    stats = running_stats.get_statistics()
    total_frames = len(valid_starts) * chunk_size
    logging.info(
        f"EE/SO3 relative action stats ({len(valid_starts)} chunks, {total_frames} frames): "
