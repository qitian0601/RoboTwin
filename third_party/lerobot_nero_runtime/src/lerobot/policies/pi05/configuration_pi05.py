...
class PI05Config(PreTrainedConfig):
    ...
    relative_action_type: str = "joint"
    ...
    # demonstration chunk, especially near the end of a trajectory.
    feature_adapter_flow_loss_weight: float = 0.05
    feature_adapter_velocity_loss_weight: float = 1.0
    feature_adapter_global_feature_loss_weight: float = 0.05
    feature_adapter_canonical_identity_loss_weight: float = 0.2
    feature_adapter_canonical_velocity_loss_weight: float = 1.0
    feature_adapter_residual_loss_weight: float = 0.01
    ...
    def __post_init__(self):
        ...
        if self.drop_n_last_frames is None:
            self.drop_n_last_frames = self.chunk_size - 1
    ...
    def validate_features(self) -> None:
        ...
        expected_dim = expected_dim_by_relative_type.get(self.relative_action_type)
        if self.use_relative_actions and expected_dim is not None:
            ...
            if state_dim != expected_dim or action_dim != expected_dim:
                raise ValueError(
                    f"relative_action_type={self.relative_action_type!r} requires "
                    f"{OBS_STATE} and {ACTION} to both be {expected_dim}D, got "
                    f"{OBS_STATE}={state_dim}D and {ACTION}={action_dim}D."
                )
