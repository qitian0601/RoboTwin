...
def _reconnect_relative_absolute_steps(
    ...
    relative_absolute_pairs = (
        ...
        (EERelativeActionsProcessorStep, EEAbsoluteActionsProcessorStep),
        (EELocalSE3RelativeActionsProcessorStep, EELocalSE3AbsoluteActionsProcessorStep),
    )
    for relative_cls, absolute_cls in relative_absolute_pairs:
        relative_step = next((s for s in preprocessor.steps if isinstance(s, relative_cls)), None)
        if relative_step is None:
            continue
        for step in postprocessor.steps:
            if isinstance(step, absolute_cls) and step.relative_step is None:
