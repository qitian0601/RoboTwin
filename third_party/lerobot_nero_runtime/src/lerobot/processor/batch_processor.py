...
class AddBatchDimensionComplementaryDataStep(ComplementaryDataProcessorStep):
    ...
    def complementary_data(self, complementary_data: dict) -> dict:
        ...
        if "action.loss_mask" in complementary_data:
            loss_mask = complementary_data["action.loss_mask"]
            if isinstance(loss_mask, Tensor) and loss_mask.dim() == 1:
                complementary_data["action.loss_mask"] = loss_mask.unsqueeze(0)

        for key, value in list(complementary_data.items()):
            if key.startswith(f"{OBS_IMAGES}.") and key.endswith(".mask"):
                if isinstance(value, Tensor) and value.dim() == 0:
                    complementary_data[key] = value.unsqueeze(0)
